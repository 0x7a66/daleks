...
public class Max extends AbstractAggFunction {
    ...
    public void calculate(AccumulateRequest request) {
        ...
        log.debug("Max.calculate, key: {}, value: {}", key, value);
    ...
    public Object getValue(AccumulateRequest request) {
        ...
        log.debug("Max.getFactorValue, keys: {}, values: {}", keys, values);
        if (CollectionUtils.isNotEmpty(values)) {
            return values.stream().mapToDouble(item -> item == null ? 0D : NumberUtils.toDouble(item)).max().orElse(0D);
        }
        return 0D;
    }
    ...
    public String function() {
        return AccumulateFunction.MAX.name();
    }
}
