...
import FunctionConfigDetail from "./components/FunctionConfigDetail";
...
export default () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'func',
      sorter: true,
      render: (text, record) => <Link to={`/strategy/function/config/detail?id=${record.id}`}>{text}</Link>
    },
    {
      title: '标签',
      dataIndex: 'tags',
        ...
        menu={[
          {
            ...
            icon: <EditFilled />,
            onClick: () => {
              history.push(`/strategy/function/config/update?id=${record.id}`);
            }
          },
          {
            key: 'delete',
  ...
  const onBatchDelete = () => {
    Modal.confirm({
      icon: false,
      title: `删除 ${selectedRowKeys.length} 个函数 ?`,
      onOk: async () => {
  ...
  return (
    <ProCard title={
      <Space size={'middle'}>
        <span>函数管理</span>
        <Button type='primary' onClick={() => {history.push('/strategy/function/config/add')}} icon={<PlusOutlined/>}>新增</Button>
