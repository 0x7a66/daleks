...
import GroupRuleDetail from "./components/GroupRuleDetail";
...
export default () => {
        ...
        menu={[
          {
            ...
            icon: <EditFilled />,
            onClick: () => {
              history.push(`/strategy/group/rule/update?id=${record.id}`);
            }
          },
          {
            key: 'delete',
  ...
  const onBatchDelete = () => {
    Modal.confirm({
      icon: false,
      title: `删除 ${selectedRowKeys.length} 个策略组 ?`,
      onOk: async () => {
  ...
  return (
    <ProCard title={
      <Space size={'middle'}>
        <span>策略组管理</span>
        <Button type='primary' onClick={() => {history.push('/strategy/group/rule/add')}} icon={<PlusOutlined/>}>新增</Button>
