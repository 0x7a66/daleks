...
import {CloseOutlined, DeleteFilled, DeleteOutlined, EditFilled, PlusOutlined, SendOutlined} from "@ant-design/icons";
...
export default () => {
        ...
        menu={[
          {
            ...
            icon: <EditFilled/>,
            onClick: () => {
              history.push(`/strategy/event/update?id=${record.id}`);
            }
          },
          {
            key: 'delete',
  ...
  const onBatchDelete = () => {
    Modal.confirm({
      icon: false,
      title: `删除 ${selectedRowKeys.length} 个事件 ?`,
      onOk: async () => {
    ...
    <ProCard title={
      <Space size={'middle'}>
        ...
        <Button type='primary' onClick={() => {
          history.push('/strategy/event/add')
        ...
        <Button type='primary' onClick={onBuild} icon={<SendOutlined/>}>构建</Button>
      </Space>
