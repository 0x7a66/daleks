...
export default () => {
        ...
        <DisplayContent>
          <Descriptions column={1} size='small'>
            ...
            <Descriptions.Item label="参数">
              <Typography.Text copyable={true} ellipsis={{tooltip: 'more'}}>{variable.parameters}</Typography.Text>
            </Descriptions.Item>
            <Descriptions.Item label="备注">
              <Typography.Text copyable={false} ellipsis={{tooltip: 'more'}}>{variable.remark}</Typography.Text>
            </Descriptions.Item>
          </Descriptions>
        </DisplayContent>
  ...
  const showQueryValue = async (id) => {
    const variable = await api.get({id})
    setVariable(variable)
    if (variable && variable.parameters) {
      const keys = await apiScript.parse(variable.parameters)
      setKeys(keys)
    }
    setDrawerVisit(true)
  }
