...
import {CloseOutlined, DeleteFilled, DeleteOutlined, EditFilled, PlusOutlined, CloudFilled} from "@ant-design/icons";
import {history, Link} from 'umi';
import AccumulateDetail from "./components/AccumulateDetail";
...
export default () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'tags',
      filters: tagFilter,
      render: (_, {tags = []}) => tags.map(tag => <Tag key={tag.id} color={tag.color}>{tag.name}</Tag>)
    },
    {
      title: '操作',
      dataIndex: 'id',
        ...
        menu={[
          {
            ...
            icon: <EditFilled />,
            onClick: () => {
              history.push(`/strategy/accumulate/update?id=${record.id}`);
            }
          },
          {
            key: 'delete',
  ...
  const onBatchDelete = () => {
    Modal.confirm({
      icon: false,
      title: `删除 ${selectedRowKeys.length} 个指标 ?`,
      onOk: async () => {
  ...
  return (
    <ProCard title={
      <Space size={'middle'}>
        <span>指标管理</span>
        <Button type='primary' onClick={() => {history.push('/strategy/accumulate/add')}} icon={<PlusOutlined/>}>新增</Button>
